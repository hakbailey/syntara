...
async def _clone_invocation(source_id: UUID) -> UUID:
...
async def _run(source_id: UUID) -> tuple[UUID, dict[str, object]]:
    ...
    set_runtime_settings(SettingsCache(session_factory=AsyncSessionLocal, redis_enabled=False))
    invocation_id = await _clone_invocation(source_id)
...
def main() -> int:
    ...
    standalone_id, result = asyncio.run(_run(args.invocation_id))
    ...
    if args.compare_to:
        ...
        if _canonical(_parity_contract(result)) != _canonical(_parity_contract(expected)):
            print(
